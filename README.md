...
- 같이가요(모집글): 참여 인원 상한, 모집 마감(날짜/시간) 설정, 알림(참여자 생기면 작성자에게 알림) 검토
- 건의사항: 상태 변경 시 작성자에게 알림, 인기순 정렬(공감 많은 순), 중복/유사 건의 병합 UI 검토
- 카카오톡 ID 교환: 요청/수락 시 상대에게 알림(현재는 ME 탭 재방문해야 확인 가능), 교환 완료 후 매너 평가 유도
...
- 알림 기능(FCM 연동): 카톡 교환 요청/수락, 같이가요 참여, 건의사항 상태 변경 등을 실시간 푸시로 알려주기
